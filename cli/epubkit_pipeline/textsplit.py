...
def _split_epub_text(path, log, result):
    ...
    for tag in re.findall(r'<(?:\w+:)?item\b[^>]*/?>', opf):
        ...
        if iid and href:
            items[iid] = (href, tag)
    spine_ids = re.findall(r'<(?:\w+:)?itemref\b[^>]*\bidref="([^"]+)"', opf)
    ...
    # Remap fragment links and drop page-list navs.
    for n in list(order):
